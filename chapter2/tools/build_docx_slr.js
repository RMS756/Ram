...
const EQ = {
  ...
  '2.2': 'p̂ = 1 / (1 + exp(A·s + B))',
  '2.3': 'a*(x) = arg minₐ  Σ_{y ∈ {0,1}}  Pr(y | x) · C(a, y)',
};
...
while (i < md.length) {
  ...
  // figure: ![caption](file.png)
  let fm;
  if ((fm = line.match(/^!\[(.*?)\]\((.*?)\)\s*$/))) {
    const img = fs.readFileSync(path.join(path.dirname(SRC), fm[2]));
    cur.children.push(new Paragraph({ alignment: AlignmentType.CENTER, keepNext: true, spacing: { before: 120, after: 80 },
      children: [new ImageRun({ type: 'png', data: img, transformation: { width: 600, height: 463 }, altText: { title: 'Figure 2.1', description: fm[1], name: 'PRISMA flow diagram' } })] }));
    i++; continue;
  }
...
const header = new Header({ children: [new Paragraph({ alignment: AlignmentType.RIGHT, children: [new TextRun({ text: 'Chapter 2 — Literature Review (Systematic Review)', font: FONT, size: 18, italics: true, color: '595959' })] })] });

const doc = new Document({
  creator: 'REM thesis',
  title: 'Chapter 2 — Systematic Literature Review',
  styles: {
